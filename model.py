...
def simulate_win_prob(shots_behind: float,
    ...
    Monte Carlo simulate your chance to win given:
      ...
      - sg_expect_round: your expected strokes-gained per 18 holes vs field
...
def calculate_score():
    try:
        ...
        # --- new manual inputs ---
        holes_left      = float(holes_left_entry.get())
    ...
    score = (
        ...
        - ranking * 0.5
        - leaderboard_pos * 0.3
        - (shots_behind_entry.get() and 0)  # placeholder, replaced below
        - avg_last5 * 0.5
        + sg_off_tee  * 0.5
        + sg_approach * 0.5
        + sg_putting  * 0.5
    ...
    # Replace the shots-behind term with holes-left scaling
    ...
    p_sim = simulate_win_prob(
        ...
        holes_left=int(holes_left),
        sg_expect_round=sg_expected,
        contenders=n_contenders
    )

    # Blend heuristic + simulation
    p_model = BLEND_MODEL * p_model + (1 - BLEND_MODEL) * p_sim
    ...
    out = (
        ...
        f"Model: {p_model*100:6.2f}%  Market: {p_implied*100:6.2f}%  "
